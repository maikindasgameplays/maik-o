CriaCartao(
    'História',
    'A Política de Distensão, levada a cabo pelo General Ernesto Geisel, visava',
    'acalmar a tensão política entre Governo e Oposição.'

)
CriaCartao(
    'Geografia',
    'A Campanha das "Diretas-Já" foi um episódio marcante ocorrido na década de 1980, constituindo um dos significativos momentos do processo histórico brasileiro conhecido como:',
    'redemocratização'

)
CriaCartao(
    'História',
    'Qual foi o impacto da Revolução Industrial?',
    'Transformou economias agrícolas em industriais.'
    

)
CriaCartao(
    'Matemática',
    'O que é o teorema de Pitágoras?',
    '"A relação entre os lados de um triângulo retângulo.'
    

)
CriaCartao(
    'Biologia',
    'Como funciona a fotossíntese?',
    'As plantas convertem luz solar em energia'
    

)
    
CriaCartao(
    'Química',
    'O que é uma reação química?',
    'Transformação de substâncias em novas substâncias'

)
CriaCartao(
    'Física',
    'Qual é a lei da gravitação universal',
    'Todo corpo atrai outro com uma força proporcional à massa'

)
CriaCartao(
    'Geografia',
    'Quais são as camadas da Terra?',
    'Crosta, manto e núcleo.'

)
CriaCartao(
    'Literatura',
    'Qual a principal obra de Machado de Assis?',
    'Dom Casmurro.'

)
CriaCartao(
    'Sociologia',
    'O que é cultura?',
    'Conjunto de hábitos e valores de um grupo'

)
CriaCartao(
    'Filosofia',
    'O que defendia Platão?',
    'A teoria das ideias e a busca pela verdade.'

)
CriaCartao(
    'Português',
    'O que é uma oração subordinada?',
    'É uma oração que depende de outra'

)
CriaCartao(
    'Física',
    'Qual é a unidade de medida da força no Sistema Internacional (SI)?',
    'Newton (N).'

)
CriaCartao(
    'Geografia',
    'Qual é o maior país do mundo em extensão territorial?',
    'Rússia'

)
CriaCartao(
    'História',
    'Quem foi o primeiro presidente do Brasil?',
    'Marechal Deodoro da Fonseca'

)
CriaCartao(
    'Química',
    'Qual é a fórmula química da água?',
    'H20'

)
CriaCartao(
    'Filosofia',
    'Qual é a principal ideia do filósofo René Descartes em sua obra "Meditações"?',
    'Resposta: A dúvida como método de busca pela verdade, culminando na famosa frase "Penso, logo existo"'

)
CriaCartao(
    'Biologia',
    'O que é um ecossistema?',
    'É uma comunidade de organismos que interagem entre si e com o ambiente em que vivem'

)
CriaCartao(
    'Literatura',
    'Qual obra é considerada a primeira do romantismo brasileiro?',
    '"Iracema", de José de Alencar.'

)
CriaCartao(
    'Geografia',
    'Qual a capital da ESpanha?',
    'Madrid'
)
CriaCartao(
    'Ciências',
    'Quantos ossos tem no corpo do ser humano?',
    '206'
)







    